...
def parse_args():
    ...
    parser.add_argument(
        '--save_per_instance',
        default="/home/wangtao/video_caption/project_v3.0/clip_score/results.json",
        type=str,
...
def main():
    ...
    # image_ids = [pathlib.Path(path).stem[:-4] for path in image_paths]
    
    # image_feats = []
    # for path in image_paths:
    #     feat = np.load(path).mean(axis=0)
    #     image_feats.append(feat)
    # image_feats = np.stack(image_feats,axis=0)
    ...
    candidates_list = [candidates[idx] for idx in candidates.keys()]

    image_paths = []
    image_ids = []
    for idx in candidates.keys():
        image_ids.append(idx)
        image_paths.append(os.path.join(args.image_dir, idx)+'.npy')
    ...
    if args.references_json:
        with open(args.references_json) as f:
            ...
            references_list = [references[idx] for idx in candidates.keys()]
    ...
    _, per_instance_image_text, candidate_feats = get_clip_score(
        model, image_feats, candidates_list, device)
