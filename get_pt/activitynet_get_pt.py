...
with open('data/sentences/trian/activitynet/activitynet_train.json', 'r') as f:
    data = json.load(f)
...
for i in tqdm(range(0, len(data[:])//batch_size)):
    ...
    with torch.no_grad():
        texts_token = tokenizer(texts, truncate=True).to(device)
...
torch.save(text_features, "activitynet_text_features.pt")
