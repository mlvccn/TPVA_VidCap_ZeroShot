...
with open('data/sentences/trian/vatex/vatex_train.json', 'r') as f:
    all = json.load(f)
data = []
for item in all:
    for cap in item['enCap']:
        data.append(cap)

tokenizer = clip.tokenize
data = random.sample(data, 250000)
text_features = []
captions = []
batch_size = 1000
clip_model.eval()
...
torch.save(text_features, "vatex_text_features.pt")